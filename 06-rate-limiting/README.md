...
is virtual — no network, no real clock, no API key. The numbers below measure
...
makespan). Admitted requests take 20–60ms and fail transiently 2% of the time.
Retry budget: base 100ms, cap 10s, max 8 retries. Same seed, fresh server per
strategy. Rejections are instant 429s carrying a Retry-After hint; whether a
strategy listens to it is part of the strategy.
...
Jitter doesn't spread load thinner here; it stops the herd from arriving as a
wall.
...
next token) finishes in 9.47s — within 5.3% of the 9.0s ideal — while every
guessing strategy takes 11–15s. The hint is deterministic, so it partially
...
into 20 and 2.96 att/ok into 1.11 — but pacing at exactly the server rate
...
6.4s; full jitter fails 2 requests and decorrelated 7, with p99 over 10s: a
