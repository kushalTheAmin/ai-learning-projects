...
|---|---------|---------|-------------------|
| 05 | [streaming-parser](05-streaming-parser/) | Incremental SSE parsing, partial JSON for streamed tool calls, and backpressure, in TypeScript | Chunk boundaries respect nothing — 54 slicings of one stream, including mid-code-point cuts, must reassemble identically; partial parsing makes the mean tool-arg field readable in final form at 48.2% of the stream, but re-parsing the prefix per delta is O(n²) (13.6 s at 67 KB), and an unbounded queue buffers 5,039 chunks where a cap-8 channel holds 8 at identical throughput. |
