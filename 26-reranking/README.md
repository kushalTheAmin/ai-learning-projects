...
bm25+maxsim@20         mrr@10 0.838   keyword 0.950   paraphrase 0.725   latent dots/query  1909.5   gold in shortlist 0.950   +3/-4
bm25+maxsim@100        mrr@10 0.825   keyword 0.950   paraphrase 0.700   latent dots/query  9298.9   gold in shortlist 1.000   +3/-4
...
bm25+maxsim@20 vs bm25                 diff -0.013   95% ci [-0.079, +0.048]   p_le_zero 0.6686
...
**same vectors, finer interaction, worse ranking.** MaxSim over per-term vectors from the very same LSA fit loses to the bm25 stage it reranks, 0.838 at depth 20 and 0.825 at full depth, demoting 4 paraphrase queries (p08, p11, p17, p18) while promoting 3. and it costs two orders of magnitude more than the pooled scorer (1909.5 dots vs 20.0). the interaction granularity was never the bottleneck, the representation was: untrained single-term LSA vectors are noisy, and one noisy term-to-term max can outvote the pooled document context that actually carried the signal. real late-interaction models train the term vectors for exactly that interaction; granularity without training just adds variance.

**keyword queries are immune to every scorer, by mechanism.** the keyword column reads 0.950 for every system in the table. an exact term match has cosine 1.0 in maxsim, so docs containing all query terms tie at exactly 1.0, and the stable sort resolves ties by first-stage order. the reranker structurally cannot damage what the lexical stage already got right, which is a property you want and normally have to engineer on purpose.

**the headroom is in the scorer, not the shortlist.** at depth 20 the shortlist already contains the gold doc for 95% of queries, and the oracle turns that into mrr 0.950. the gap between the oracle and the best real scorer, +0.089 with ci [+0.025, +0.160], is the only gap in the table that clears zero. so on this corpus a better shortlist buys almost nothing (deepening 20 to 50 moves the oracle 0.950 to 1.000) while a genuinely better pair scorer has 9 points of mrr sitting on the table. thats the case for a trained cross-encoder, priced without simulating one.
...
- the negative maxsim result is about untrained term vectors, not late interaction in general. its the control condition colbert-style training exists to fix.
...
- maxsim spends 1909.5 dots at depth 20, mostly on low-idf doc terms that can never win a weighted max. how much of that cost does idf-pruning doc profiles recover before mrr moves?
