...
|---|---|---|---|
| 08-agent-tool-loop, result extension | 2026-09-04 | typescript | tool-result validation boundary answering 08's garbage-results thread: per-tool zod result-value schemas (bare integer, finite number string, status line, nonempty prose) + loop-level result validation policy (off / reject / rerun-N with a per-call rerun budget and a bad-result fail reason; all prior tables byte-identical with validation off) + per-task corruption wrapper on one tool (lie x10, empty string, html garbage, 20000-char bomb, transient-garbage clearing on rerun) + {last}/{last:number} arg substitution carrying the previous tool value into the next call's args so corruption propagates; measured on 11 tasks x 3 policies: mid-chain garbage is caught anyway but one hop late at 22x the tokens (Number(garbage) is NaN, the next calc call fails arg validation and the loop guard kills the task at 374 tokens-in vs 17 at the boundary), the empty string slips even that net (Number("") is 0, a valid arg, ships "doubled population: 0" with no validator firing), the bomb bills 15341 tokens-in with validation off vs 17 rejected unseen (902.4x, every later call re-reads it from history), last-hop garbage ships as the final answer because no next call's arg schema exists to object, reject-vs-retry separates detection from rescue (transient garbage: both catch it, only retry2 rescues 2/2 at 1 rerun each), and the two shapes that actually ship wrong answers, a plausible lie at exactly clean-run cost (168 tokens-in, same as clean) and corrupted free text, pass every schema under every policy |
| 06-rate-limiting, rolling extension | 2026-09-03 | typescript | rolling-window error-rate breaker answering 06's rolling-window thread on the unchanged scenarios: shared gate machinery extracted into GatedBreaker (one open/half-open/single-probe state machine with a pluggable trip detector; CircuitBreaker behavior pinned by the untouched 139-test suite and byte-identical breaker-study output) + RollingWindowBreaker (failure rate over a (now - windowMs, now] settle window with lazy eviction behind a minVolume floor, successes dilute instead of resetting, a trip discards the window, a probe success closes it empty) + detector-vs-detector reruns of the dead-service, herd-429, and survivable-outage studies + a 20-seed chance-streak sweep on a healthy high-capacity server: consecutive k=5 shared false-trips 17/20 runs at 30% transient faults (52.7% mean success, 189.1 fast-fails/run, every one a request 9 attempts would have saved) vs rolling 50%/1s/v20 at 3/20 and 90.8%, both 20/20 on the 70% brownout; per-client the rolling detector is volume-starved inert (exact no-breaker 1080-attempt bill), counting 429s it trips on the herd burst's genuine 50% rate identically to the counter, and shared wait mode spends one probe per cooldown herd-wide so 40 clients outlive the 30s outage 100% vs plain 0% at makespan 31.11s |
| 17-confidence-calibration, signals extension | 2026-09-03 | python | selective-prediction trust-signal study answering 17's which-signal-ranks-mistakes-best thread on the unchanged model, seeds, and traffic: three cheap per-row signals oriented higher-is-trusted (max softmax, probability margin top1 minus top2, negative entropy with 0 log 0 = 0) + temperature-invariant logit margin (T scales every top-two logit gap uniformly, pair disagreement under scaling asserted exactly 0) + risk-coverage machinery (deterministic index-tie-broken trust ordering, prefix error-rate curve, aurc as its mean, closed-form oracle aurc floor at a given accuracy, widest-prefix coverage-at-risk operating point, risk-at-coverage) + pairwise ordering-disagreement rate (fraction of item pairs two signals order strictly oppositely, ties not conflicts); measured — the signals nearly tie: orderings disagree on under 2% of pairs (msp vs margin 0.0075, vs entropy 0.0095, margin vs entropy 0.0169) and aurc reads 0.0675 / 0.0680 / 0.0673 against an oracle floor of 0.0227, so picking between cheap signals is a third-decimal decision while two thirds of the achievable mistake-ranking is information no function of one probability row carries; at a 5% error budget they answer 55.2% / 53.2% / 55.5% of test traffic; the fitted T=3.060 reorders cross-row (msp 4.05% of pairs, entropy 7.09%, margin 1.62%) and improves every aurc a hair (0.0675 to 0.0660), so calibration mildly helps the ranking too; under the drifted stream the budget coverage collapses 55.2% to 2.3% (28 of 1200) while every score still reads as a probability, a trust signal is not a shift detector; the largest msp-vs-margin rank split is a 0.519 / 0.189 / 0.171 / 0.121 row, the spread-thin-doubt shape that separates the signals and settles nothing measurable here |
| 05-token-streaming, sse-limits extension | 2026-09-03 | typescript | sse buffer caps answering 05's unbounded-line thread on the unchanged parser semantics: SseLimits (maxLineChars for the incomplete line, maxEventChars for in-flight event data, both in utf-16 code units of the decoded stream, the unit the retained strings occupy memory in) + two deliberate failure modes (error: typed SseLimitError poisons the parser, every later call rethrows; skip: drop the offending line or event, count it in stats, keep the stream) + discard state machine surviving crlf split across feeds and empty feeds, with a chunk-boundary trailing cr excluded from the line length so an at-cap crlf line never false-trips + retained-chars high-water stat; measured: an unterminated 4194304-char line retains 4194310 chars uncapped (0 events, held forever) vs error mode failing at 66560 chars fed (1.6% of the poison) vs skip mode delivering all 40 healthy events around a 1 MiB poison line at high-water 65650 against the cap+chunk bound 66560; 2100 terminated short lines still accumulate 1077299 chars past the line cap and the event cap holds high-water 65803; 300/300 seeded chunkings of the well-formed fixture byte-identical to the uncapped parser |
| 09-concurrency, breaker extension | 2026-09-03 | typescript | breaker-vs-budget study answering 09's breaker-vs-budget thread on the unchanged storm grids: 06's CircuitBreaker imported unchanged as one fail-fast gate shared by every task (a rejected gate ends the task with zero wire attempts and zero budget spent; wait mode deliberately absent since open-loop arrivals would only re-queue the storm client-side without shedding its volume arithmetic) + client-evidence gate settlement (an attempt's gate settles exactly once at the instant the client learns the outcome, a timeout counting as a failure right then, orphan server-side results never touching the breaker) + fastfail/trips/probes/probe-failure/rejection accounting with baseline rows pinned byte-identical to the storm extension's published table + cooldown sweep + trip-threshold sweep under flake; measured — the breaker dominates the dip outright (unbudgeted jitter NEVER at amp 4.10 and $33.21/1k becomes 85.2% ok, 0.0s recovery, $1.89, beating no-retry's 66.9%/15.0s/$2.73 because shed arrivals never orphan into the post-dip queue; the three breaker rows print one headline row because retries die at the gate, budget denials 897 → 34), the half-open probe does reopen the storm mid-dip (a drained server answers one 500ms probe under the 1s timeout, the breaker closes, 25/s lands on 8/s and re-trips within five timeouts: trips 2 at 5s cooldown, 3 at 1s with 3/6 probes failing, 1 at 15s which sheds 18.0% of tasks including healthy post-dip arrivals) but the flap is priced small and the cooldown forgiving anywhere in 2-5s, sustained overload becomes duty-cycled load shedding (104% load: 23.2% → 78.6% ok, 125%: 4.1% → 37.8% at amp 0.49 and 9 trips) with p95 unmoved at ~950ms because survivors still ride a queue at the timeout boundary, and the trip threshold is a knife edge on healthy-but-flaky traffic: at 20% iid flake that jitter-x4 alone absorbs at 100.0% ok, threshold 3 trips 8 times and sheds 1180 healthy tasks for 47.6% ok while threshold 5 never fires on this seed |
| 16-llm-as-judge, power extension | 2026-09-03 | typescript | detection-power study answering 16's single-call thread on the unchanged judges and lean statistic: text-free power pair generator (core-set quality draws, answers pinned at the length pivot, gold-in-a share exact in tenths for balance and skew sets) + three lean detectors at a shared call budget (both-order on budget/2 pairs, randomized single-call on budget pairs, single-call at half budget on the same budget/2 pairs) + replicated power harness (500 seeded experiments per cell, two-sided threshold calibrated as the empirical 95th percentile of \|lean\| on the bonus-0 null, power = strict-exceedance rate, min detectable bonus at 80%) + null-sd variance ratio as the equal-power call multiplier + arrangement-skew control at authored bonus 0; the reframe: a randomized single call coin-flip picks one of the pair's two both-order calls, so both statistics estimate the same quantity and the whole question is variance — a clean pair contributes exactly 0.5 first-wins under both-order (zero variance, canceled by construction) but a full Bernoulli order draw under a single call, 02's paired-vs-unpaired story on judge calls; measured — at sigma 0.04 the variance ratio is 43.2x (100 calls) and 44.8x (400): both-order detects the cast's own 0.15 bonus 100% and 0.09 at 82.2% on 50 pairs where single-equal reads 68.2% and 16.4%, min-80% bonus at 400 calls 0.06 vs 0.12 vs 0.15 (single-half); at 3x noise the ratio collapses to 3.2x/2.8x because noise makes every pair a swing pair, and single-equal's power at fixed bonus more than doubles (0.164 to 0.362 at bonus 0.09) while both-order barely moves (0.822 to 0.880), the direction extension's more-noise-reads-more-lean now visible as detection probability, so both-order is most valuable exactly when the judge is good; the skew control reads as-stored single-call lean 0.395 of pure fiction at a 90/10 arrangement with zero authored bonus (randomized 0.006, both-order 0.000), and both-order's low-noise null is discrete enough to realize 0.002 false positives against nominal 0.05, a conservatism it dominates through anyway |
| 11-prompt-caching, position extension | 2026-09-02 | typescript | volatile-position study answering 11's position thread on the unchanged cache, pricing, and workload generator: constant-size 120-token per-request volatile block inserted at a swept absolute index (clamped to sit at latest directly before the current user message, TAIL sentinel for always-last, staticPrefixEnd shifted when the block lands at or before it) + volatile-aware breakpoint strategy (deepest stable index directly before the known volatile block plus the surviving static prefix, nothing at or past it since such entries can never be read back) replayed against the oblivious incremental combination, each position priced against its own no-caching baseline through a genericized replay; measured on 12 turns x 4 history blocks/turn at 30s gaps — the oblivious operator prints one flat row at every position past the static prefix (0.886x, 31.7% hits, 31625 write tokens nobody ever reads) and exactly 1.250x with 0 hits when the block leads the request, the aware curve falls monotonically 1.000x at position 0 to 0.722x at position 2 to 0.301x at the tail against the 0.255x stable floor (a 2.4x swing, $0.0668 vs $0.0279, from assembly order alone), and at position 1 the lone stable prefix (tools, 882 tokens) sits under the 1024-token min-cacheable floor so the aware strategy's only legal breakpoint silently caches nothing and prints exactly 1.000x |
| 09-concurrency, cancel extension | 2026-09-02 | typescript | cancellation propagation answering 09's cancellation thread on the unchanged storm grids: AbortSignal-aware FIFO semaphore acquire (abort while waiting dequeues the waiter and rejects with AcquireCancelledError, a granted permit ignores its signal, the listener is detached at permit handoff) + cancellable simulated-api call (a call aborted while queued for a slot is never admitted or charged, ApiError kind cancelled, cancelled-in-queue and max-queue-depth stats) + cancelOnTimeout storm mode decomposing abandoned attempts into cancelled vs wasted completions; measured on the identical dip and cliff grids: capped policies rescued (no-retry 66.9% to 84.4% ok, recovery lag 15.0s to 0.0s, post-dip queue 259 to 25; jitter+budget10 60.1% to 84.5%), unbudgeted retry storms survive cancellation (all still NEVER at amplification 4.10, volume arithmetic untouched) but the bill collapses (wasted 94.5% to ~27%, drain 250.1s to under 102s, $33.21 to ~$10 per 1k completions); cancel mode bounds queue depth at ~admission rate x timeout, and at sustained overload most doomed work is next in line rather than queued, so only the overflow reclaims |
...
unlanded, deletion stays a by-hand task on the github branches page. this
run's own session branch existed only locally and was removed.
Re-attempted 2026-09-02 (11-position run): still denied, same sideband
disconnect on ref deletion. unchanged: nothing on those branches is
unlanded, deletion stays a by-hand task on the github branches page. this
...
- [fixed 2026-09-04] 05 — measurement 5's `speedup` column is a ratio of two
  wall clocks and the readme prices its variance at "move a few percent run
  to run". it does not. four runs of the same committed code on the same
  machine gave the 8273-char row 57.7x (the committed number), 110.1x, 69.6x
  and 52.8x, a 2.1x spread, and the 65619-char row 662.5x, 623.6x, 491.4x,
  503.9x. the root readme's index row carries the same two endpoints as
  headline facts, "3.4x faster at 266 chars rising to 662.5x at 64KB", and
  the projection sentence ("~629.2s by the n² law") is derived from the same
  unstable baseline median — observed 485.5s and 514.0s on re-runs. the
  takeaway the section actually supports is safe (the resumable scanner wins
  at every size and the gap grows superlinearly) and it already prints the
  exact, deterministic evidence for it: 172521764 chars scanned vs 65619,
  2629.1x. five runs on this machine confirmed the spread from the other
  direction, 40.5x to 60.1x at 8273 and 452.7x to 524.8x at 65619, so the
  committed 662.5x sits outside the band the code produces here at all.
  fixed by pricing the section in work rather than in clock. the entry point
  now prints two blocks: chars fed through a scanner per replay, which is
  exact and identical every run (3365 / 47479 / 2721000 / 172521764, against
  a resumable scan of the document length at every size), and then the wall
  clock, labelled run-dependent and with no ratio column at all. new
  `baselineScanChars(docChars, seed, maxFragmentChars)` in resumableBench.ts
  sums the prefix lengths the baseline reparses straight off `chunkOffsets`,
  so the count is exact without running the replay — which is what lets the
  1048586-char row state 44074751901 scanned chars, 42032.6x the document,
  counted rather than projected in seconds off a median that moved 485.5s to
  629.2s across runs. both readmes rewritten to lead on the counts; the root
  index row trades its two speedup endpoints for the 172521764-vs-65619
  contrast. new tests/benchClaims.test.ts, 12 tests: five hold
  `baselineScanChars` to what a real baseline replay actually scans (three
  published sizes, eight seed/cap combinations, the prefix-sum identity, the
  1-char-per-fragment quadratic worst case, empty and single-char documents)
  and pin the two counts the readmes publish including the one too slow to
  run, and seven hold the prose — the speedup column and its figures gone
  from section 5, the "few percent" variance claim gone, the four exact table
  rows present, the 1MB row stated as counted work, the spread named as
  observation not bound, and the root index row carrying the counts. all 12
  fail on the old code; the revert check in a clean clone reverts src/ and
  both readmes and fails exactly those 12 with all 129 pre-existing green. no
  measured number moved — every exact figure in the section is the one it
  always was, only the unstable ones are gone. 129 tests → 141.
- [medium] 02 — same mechanism as the 05 speedup column, one project over,
  and left alone this run because the change is not the identical one. the
  corpus-size sweep and the query-cost strata both carry a `speedup` column
  that divides two wall clocks, published at two to four significant figures
  — 2.8x through 5.7x down the sweep, and 382.6x for the rare-only stratum,
  which the prose then quotes twice as the headline ("the inverted index gets
  its 382.6x precisely there"). the 1M-doc projection ("~2.0s/query flat vs
  ~0.36s/query inverted") is a slope fitted to the same clocks. what makes it
  weaker than 05's was: 02 already prints the deterministic evidence beside
  it — postings/q, scanned/q, candidates/q, and a touch ratio that sits flat
  at 2.8x across the whole sweep — and the load-bearing bullet leans on the
  touch ratio, not the clock. so the reading survives even if every ms column
  moves. the drift itself is unmeasured here: i did not re-run 02 this pass,
  so i cannot say whether its ratios move 10% or 2x run to run, and that is
  the first thing to measure next time rather than assume 05's spread carries
  over. note 02 is also the project whose 1M-doc projection is already filed
  below as quoted to two significant figures off a fitted slope — same
  sentence, two findings, worth fixing together. found 2026-09-04.
...
- sse buffer limits: line and event caps in decoded utf-16 code units, error mode with a typed poisoning SseLimitError, skip mode with a discard state machine (crlf split across feeds, pending-lf swallow across empty feeds, chunk-boundary trailing cr excluded from the line length) and dropped-line/dropped-event stats (05, typescript)
- retained-state high-water accounting for a streaming parser: incomplete line plus in-flight event data, sampled per append and per drain (05, typescript)
- authored hostile sse wires (unterminated line, giant terminated frame, one event of thousands of terminated data lines) with a fixed-chunk replay harness reporting deliveries, drops, and retention (05, typescript)
...
- per-tool result-value zod schemas + loop-level result validation policy with per-call rerun budget and bad-result failure (08, typescript)
- tool-result corruption wrapper: lie/empty/garbage/bomb/transient-garbage families, one tool per task, transient clears on re-execution (08, typescript)
- last-value arg substitution ({last}/{last:number}) propagating tool results into later call args in the scripted model (08, typescript)
...
- per-request volatile block insertion at a swept index: constant-size unique-text block, clamp to the pre-user tail, TAIL sentinel, static-prefix-end shift (11, typescript)
- volatile-aware breakpoint placement: deepest stable index directly before a known volatile block, nothing marked at or past it (11, typescript)
- cost-of-volatility position curve: per-position cost ratio under oblivious vs volatile-aware placement against a stable-render floor (11, typescript)
...
- replicated detection-power harness: empirical two-sided threshold from the effect-0 null (95th percentile of the absolute statistic), strict-exceedance power grid over authored effect sizes, min detectable effect at a target power, null-sd variance ratio as the equal-power call multiplier between paired and unpaired designs (16, typescript)
- single-call randomized and as-stored position-lean estimators beside the both-order lean, with a text-free exact-skew pair generator (gold-in-a share in tenths) for the arrangement-confound control (16, typescript)
...
- selective-prediction trust signals: probability margin (top1 minus top2), negative entropy with 0 log 0 = 0, and temperature-invariant logit margin, all oriented higher-is-trusted (17)
- risk-coverage curve with deterministic index-tie-broken trust ordering, aurc as its mean, and a closed-form oracle aurc floor from the accuracy alone (17)
- coverage-at-risk operating point (widest trusted prefix inside an error budget, risk not monotone in coverage so the widest qualifying prefix wins) and risk-at-coverage (17)
- pairwise ordering-disagreement rate between two score vectors, ties counted as non-conflicts (17)
...
- abortable FIFO semaphore acquire: an AbortSignal aborting mid-wait dequeues the waiter and rejects with AcquireCancelledError, an already-aborted signal rejects without queueing, a granted permit ignores its signal, the abort listener is detached when a queued waiter is handed the permit (09, typescript)
...
- shared fail-fast circuit-breaker gate in the storm loop: 06's CircuitBreaker imported unchanged, one gate for the whole open-loop arrival population, a rejected gate ending the task with zero wire attempts and zero budget spent (09, typescript)
- client-evidence gate settlement under abandonment: the breaker gate settles exactly once at the instant the client learns the attempt's outcome, a timeout counted as a failure at the timeout instant, orphan server-side results never settling (09, typescript)
- breaker flap accounting on storm runs: trips, probes, probe failures, gate rejections, fast-failed task count and share (09, typescript)
- breaker cooldown sweep and trip-threshold-under-flake sweep against fixed storm grids (09, typescript)
...
- pluggable-detector breaker gate: shared open/half-open/single-probe machinery (GatedBreaker) with consecutive-count and rolling-rate trip detectors behind one acquire/settle contract (06, typescript)
- rolling-window error-rate trip detector: settle window over (now - windowMs, now] with lazy head-pointer eviction, minVolume floor before the rate is judged, successes dilute rather than reset, trip discards the window (06, typescript)
- multi-seed false-trip aggregation: tripped-run fraction, mean trips, mean fast-fails and mean success over a fault-rate x detector grid (06, typescript)
...
- 05: the sse limits count decoded utf-16 units, so a hostile chunk is fully decoded before any cap sees it — a byte cap ahead of the decoder refuses work earlier and joins the raw-bytes fused-scanner thread, and what decoding a poison stream itself costs is unmeasured
- 05: skip mode counts drops but delivered events dont say which of them lost a line — in-band drop signaling (an error event carrying the drop position) changes the consumer contract, and whether a damaged-but-flagged event is usable at all is the design question
- 05: the sse caps are hand-picked constants — setting them from an observed frame-size distribution with a false-drop rate attached to the chosen quantile needs a corpus of real provider streams, same gap as the replay-a-real-stream thread
...
- 06: the rolling detector's volume floor blinds low-traffic callers by design (the inert per-client row cost the full no-breaker bill) — a hybrid that falls back to consecutive counting below the floor covers both regimes, and what it re-imports of the streak false-trip problem is measurable on the existing streak grid
- 06: the 50% rolling threshold was picked once and 3 of 20 runs still tripped at 30% faults — sweeping errorRateThreshold against the fault-rate axis would draw the false-trip/detection frontier that single column only samples, and it is the 06 half of 09's seed-swept trip-rate-map thread
- 06: the streak study's failures are iid across the shared settle stream, the kindest case for a rate detector — correlated failures (one bad shard hit by a subset of clients) concentrate in the window and in the population rate differently; per-dependency windows vs one per-population window is the production layout question
- 06: shared wait mode outlives every outage on the grid (100% at 30s vs plain 0%) by hanging without bound, one probe per cooldown herd-wide — a wall-clock deadline on top would cap hang time and re-lose the longest outages on purpose, sharpening the existing deadline-budget thread
...
- 08: free-text results have no shape to check, so corrupted prose ships a wrong answer under every result-validation policy — the missing layer is a groundedness-shaped check of the result against the query (12's machinery), priced per tool call
- 08: lies pass every schema by construction — a range/sanity layer on numeric results (a city population times 10 lands outside any real city) would catch gross lies at an unmeasured false-positive cost on honest outliers
- 08: transient garbage clears after exactly one rerun, so retry2 wins by construction — with rerun success drawn from a distribution the rerun budget is a quantile of it, the same survival-analysis shape as the guard-limit thread
- 08: the bomb result is rejected outright at 20000 chars — truncating it instead would keep the call's information at a bounded token price, and the truncate-vs-reject crossover is unmeasured
...
- 09: the cancel extension's abort is free and instant, an upper bound on reclamation — a parameterized abort latency (the abort races admission over a network hop) would draw the decay curve from these numbers down toward abandon mode, and where real stacks sit on it decides whether the plumbing pays
- 09: in-service work is fully unkillable in the cancel extension, but a streaming generation is partially killable (killing saves the remaining output tokens, the prefill is spent either way) — a partial-refund cost model would shrink the wasted-work gap between the modes by exactly the killable fraction
- 09: cancel mode's successes run to 12s p95 across attempts, a tail only a per-task deadline would cap — sharpens the existing fixed-per-attempt-timeout thread with a measured cost
- 09: cancel mode bounds queue depth at roughly admission rate x timeout, which the server could enforce unilaterally by shedding anything older than the client timeout at admission — client-side cancel vs server-side age-based shed on the same grid is unmeasured, and recomposes toward the bounded-queue thread
- 09: the breaker extension's false-trip verdict is one seed (threshold 5 never fired at 20% flake, and the expected trip count over many seeds is not zero) — a seed-swept trip-rate map over the threshold x flake-rate grid would turn the t3-vs-t5 knife edge into a curve, and it is the measurement 06's rolling-window-breaker thread should be judged against
- 09: storm probes ride real arrivals, so a quiet client never probes and a busy one spends a user's request probing — a synthetic probe on its own schedule decouples the two at its own cost, unmeasured
- 09: the storm breaker is one gate for the whole population; per-client breakers would learn ~40x slower during the dip, and 06's shared-vs-per-client scope comparison predicts but does not measure that gap under open-loop arrivals
- 09: the breaker cooldown is fixed; exponential open-window backoff on repeated failed probes helps the dead-service case and hurts the short dip, and the crossover is unmeasured
- 09: wait mode was deliberately absent under open-loop arrivals, but a closed-loop caller population (each caller must get an answer before its next request) is the regime where waiting for the probe window competes with failing fast, and the storm arithmetic changes shape entirely there
...
- 11: the aware strategy assumes one known fixed volatile index — real state blocks grow and move as the request is re-rendered, and a block moving from index j to j+1 poisons position j retroactively (the old volatile bytes are baked into a different prefix); the cost of a moving volatile block, and whether tracking it online is worth a breakpoint slot, is unmeasured
- 11: the oblivious operator under mid-request volatility still saves 11.4% because the static prefix is 1333 tokens of a growing conversation, while experiment 5 showed the same strategy at 1.072x when writes dominate — the static-share at which oblivious caching crosses 1.000x is unswept
- 11: tail rendering is free for the cache but moves state content far from the system prompt — whether the model attends to tail state differently than head state is a behavior question the cost simulator cannot see
...
- 16: the null-variance ratio between single-call and both-order lean runs 43.2x at noise 0.04 and 3.2x at 0.12 and must fall toward 1x as noise grows without bound; the crossover curve between is unswept, and its shape prices both-order once a team knows its judge's flip rate
- 16: both-order's low-noise null is discrete enough that the empirical threshold realizes 0.002 false positives against a nominal 0.05 — an exact binomial test on net flip counts would spend the whole alpha, and the power it recovers at small bonuses is unmeasured
- 16: both power designs spend the budget on distinct pairs; repeat single calls on fewer pairs with fresh noise draws (what temperature sampling gives a real judge for free) is the third design, and where it lands between single-equal and both-order is unmeasured
...
- 17: the signals extension leaves two thirds of the achievable mistake-ranking on the table (best aurc 0.0673 vs oracle 0.0227) and no function of one probability row can close it — signals that look outside the row (distance to nearest training ticket, vote disagreement across k models, an ood score) are the candidates, each with a real inference cost attached
- 17: the three signal aurcs sit within 0.0007 of each other, inside plausible sampling noise on 1200 items — 02's paired bootstrap over per-item ordering contributions would say whether entropy's hair of an edge is real (same unrun machinery as 17's ece-noise thread)
- 17: shift collapsed the 5%-budget coverage 55.2% to 2.3% while every score still read as a probability — the answer rate itself is a free shift alarm, and how fast a windowed coverage estimate detects onset vs its false-alarm rate is the same lag/variance tradeoff as 17's online-refit thread
