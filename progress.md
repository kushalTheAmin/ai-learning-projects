...
|---|---|---|---|
| 06-retry-backoff | 2026-08-26 | typescript | discrete-event simulation (binary-heap event queue keyed time+insertion order, virtual clock) + token-bucket rate limiter with continuous refill, burst cap, integer retry-after hints, scripted outage window + retry policy family (immediate, fixed, expo, equal/full/decorrelated jitter, retry-after exact/jittered) under a uniform per-client attempt budget with non-finite-delay hard failure; herd metric = same-instant retry collisions (1s arrival buckets provably can't see sub-second sync); measured — 500-client herd on 50/s: no-jitter p95 151s vs fixed-2s 18s at equal attempts (7% capacity used), any jitter drops collisions 450→1 and attempts 2750→1285-1905; outage recovery: exact retry-after re-herds 200 arrivals into the recovery second, hint+jitter cuts attempts 648→494; dead service: immediate fires 1000 requests in one instant vs 254-409 in 5s for backoff, at a 2-3 min time-to-report cost | 05-token-streaming | 2026-08-26 | typescript | incremental sse parser over raw bytes (whatwg subset: lf/crlf/bare-cr incl. cr on a chunk boundary, streaming utf-8 decode, multi-line data, last-event-id, dispatch only on non-empty data) + partial-json prefix parser (single left-to-right scan: container stack, in-progress token, safe-index truncation; unambiguous completion — close string, tru->true, trim 12. to 12 — else drop the dangling piece) + bounded async queue whose await-push blocks when full, with high-water/stall instrumentation; measured — first text at 2.5% of a buffering client's wait, tool-arg fields readable at 44-89% of stream bytes vs 100% waiting for the closing brace, bounded(8) queue holds high-water at 8 chunks vs 419 unbounded at identical wall time, 300/300 seeded byte-level chunkings parse identically |
...
- discrete-event simulation: binary-heap event queue, (time, insertion order) ordering, virtual clock (06, typescript)
- token-bucket rate limiter: continuous refill, burst cap, integer retry-after hints, outage window (06, typescript)
- retry backoff policies: immediate, fixed, exponential with overflow-clamped exponent, equal jitter, full jitter, decorrelated jitter, retry-after exact and jittered (06, typescript)
- hard retry budget: per-client maxAttempts plus non-finite-delay immediate give-up (06, typescript)
- same-instant retry collision metric vs per-second arrival histogram (06, typescript)
- linearly interpolated percentile (06, typescript — sanctioned cross-language duplicate of 02's; same interpolation semantics, pinned by tests in both)
- seeded prng reuse: 06 imports 05's mulberry32 across folders instead of rewriting (06, typescript)
...
- 06: no circuit breaker — scenario 3 shows backoff stretches time-to-report to 2-3 minutes against a dead service; a breaker (failure-rate threshold, open/half-open) would cap both load and report latency, measurable in the same harness
- 06: per-client maxAttempts caps one client, not the fleet — a shared retry budget (retries as a fraction of fleet requests) is the production-grade version, unmeasured
- 06: arrivals are all-at-once or uniform-spread — Poisson/bursty arrivals would say whether the herd findings survive realistic traffic, and the sim already supports arbitrary start times
- 06: the server model has no partial degradation — real recoveries ramp capacity, and exact retry-after's stampede should hurt much more against a warming server than against this instant-recovery one
