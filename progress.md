...
- 06: the 429 is one bit of feedback — real apis also send Retry-After and ratelimit-remaining headers; a controller that reads the remaining budget could skip most of the sawtooth, and how much of the 10.9% oracle gap it recovers is measurable in this harness
...
- 06: the oracle failing 1.1% says the informed optimum sits below 100% of the advertised rate — a headroom sweep (oracle at 90/95/100%) on the drop scenario would price the safety margin directly, the knee measured from the safe side
...
- 22: the overlap score could price its own retrieval: a low best-sentence score at k=3 triggering a retry at higher k instead of a refusal would spend tokens only on the queries the score says need them — both ingredients (the k sweep, the score on the wire) are measured, the policy is unbuilt
